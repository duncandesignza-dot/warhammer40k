...
# tools/data_changes.py) put the full lists back together when they load the file.
def pack(result):
    by = {f["id"]: f for f in result["factions"]}
    for f in result["factions"]:
        p = by.get(f.get("parent"))
        if not p: continue
        pu = {json.dumps(u, sort_keys=True) for u in p["units"]}
        cu = {json.dumps(u, sort_keys=True) for u in f["units"]}
        pdk = {json.dumps(d, sort_keys=True) for d in p.get("dets", [])}
        cdk = {json.dumps(d, sort_keys=True) for d in f.get("dets", [])}
        names = [u["n"] for u in p["units"]]
        assert len(names) == len(set(names)), f"{p['name']} has two units with the same name"
        f["ux"] = sorted(u["n"] for u in p["units"] if json.dumps(u, sort_keys=True) not in cu)
        f["dx"] = sorted(d["n"] for d in p.get("dets", []) if json.dumps(d, sort_keys=True) not in cdk)
        f["units"] = [u for u in f["units"] if json.dumps(u, sort_keys=True) not in pu]
        f["dets"] = [d for d in f.get("dets", []) if json.dumps(d, sort_keys=True) not in pdk]
        for k in ("ux", "dx", "dets"):
            if not f[k]: del f[k]
    result["packed"] = 1
pack(result)
# Put each chapter's full lists back together as the file loads: the parent's (minus ux and dx) plus its own,
# sorted the way units_for() and detachments_for() sort them.
UNPACK = """(function(D){
  if(!D || !D.packed) return;
  var ROLES = %s, by = {}, cmp = function(a, b){ return a < b ? -1 : a > b ? 1 : 0; };
  D.factions.forEach(function(f){ by[f.id] = f; });
  D.factions.forEach(function(f){
    var p = by[f.parent]; if(!p) return;
    var ux = f.ux || [], dx = f.dx || [];
    f.units = p.units.filter(function(u){ return ux.indexOf(u.n) < 0; }).map(function(u){ return JSON.parse(JSON.stringify(u)); }).concat(f.units)
      .sort(function(a, b){ return ((a.t ? 1 : 0) - (b.t ? 1 : 0)) || (ROLES.indexOf(a.r) - ROLES.indexOf(b.r)) || cmp(a.n, b.n); });
    var dets = (p.dets || []).filter(function(d){ return dx.indexOf(d.n) < 0; }).map(function(d){ return JSON.parse(JSON.stringify(d)); }).concat(f.dets || [])
      .sort(function(a, b){ return cmp(a.n, b.n); });
    if(dets.length) f.dets = dets;
    delete f.ux; delete f.dx;
  });
  delete D.packed;
})(window.LEDGER_FACTIONS);
""" % json.dumps(ROLE_ORDER)
