...
(function(){
  ...
  // One army's backup: its units and photos, and its army lists and battle reports.
  async function downloadArmyBackup(army, units){
  ...
  async function confirmDeleteArmy(army, units, onDeleted){
    const [ls, gs] = await Promise.all([store.listLists().catch(() => []), store.listGames().catch(() => [])]);
    const nl = ls.filter(l => l.armyId === army.id).length, ng = gs.filter(g => g.armyId === army.id).length;
    const goes = [units.length ? `its ${plural(units.length, "unit")}, with their photos, colours and recipes` : "", nl ? plural(nl, "army list") : "", ng ? plural(ng, "battle report") : ""].filter(Boolean);
    const d = modal(`Delete ${esc(army.name)}?`, `
      <p>This deletes ${esc(army.name)} from Livery Ledger and War Ledger${goes.length ? `: ${esc(goes.join(", ").replace(/, ([^,]*)$/, " and $1"))}` : ""}. It can't be undone.</p>
      <p class="dl-tip">Want a copy first? <button type="button" class="linkish" id="dl-export">Download a backup</button> before you delete.</p>
      <div class="row-actions"><button type="submit" class="danger armed" id="dl-go">Delete everything</button><span class="spacer"></span><button type="button" id="dl-cancel">Cancel</button><span class="msg" id="dl-msg" role="status"></span></div>`);
    $("dl-cancel").addEventListener("click", () => d.close());
    $("dl-export").addEventListener("click", async () => { await downloadArmyBackup(army, units); $("dl-msg").textContent = "Backup downloaded."; });
    d.querySelector("form").addEventListener("submit", async e => {
      e.preventDefault();
      const b = $("dl-go"); b.disabled = true; b.textContent = "Deleting…";
      try { await store.removeArmy(army); d.close(); flash(`Deleted ${army.name}`); onDeleted(); }
      catch(err){ $("dl-msg").textContent = "Couldn't delete: " + errText(err); b.disabled = false; b.textContent = "Delete everything"; }
    });
    $("dl-cancel").focus();
  }
  ...
  async function restoreBackup(b, {into = null, progress = () => {}} = {}){
    ...
    for(const L of b.ledgers){
      ...
      armyMap[L.id] = target; if(!out.firstArmy) out.firstArmy = target;
  ...
  const toolName = () => isWar() ? "War Ledger" : "Livery Ledger";
  function setMode(m){
  ...
  // Models and points in some units. War Ledger counts every unit, owned or not, so you can try out forces you don't have.
  const modelsOf = u => Math.max(1, +u.count || 1);
  const sumUp = units => units.reduce((t, u) => { t.models += modelsOf(u); t.points += +u.points || 0; return t; }, {models: 0, points: 0});
  ...
  const dayText = d => { if(!d) return ""; const t = new Date(d + "T12:00:00"); return isNaN(t) ? d : t.toLocaleDateString("en-GB", {day: "numeric", month: "short", year: "numeric"}); };
  ...
  function stopTimer(){
    ...
    askTime(`Add painting time to ${esc(t.name)}`, mins, async add => {
      if(add){
        const u = (await store.listUnits(t.armyId)).find(x => x.id === t.unitId) || (await store.listAllUnits()).find(x => x.id === t.unitId);
        if(!u){ setTimer(null); flash("That unit has been deleted, so the time wasn't saved."); return; }
        const row = await store.saveUnit(u.armyId, mergeUnit(u, {tlog: addTime(u.tlog, add)}), u.id, null, false, u);
  ...
  // Everything War Ledger shows: armies, units, lists and battles.
  async function warData(){
  ...
  const warEmpty = () => `<section class="panel war-empty">
      ...
      <p class="sub">Pick a faction and name your force. Then add units, whether you own them or not, or paste an army list to add them all at once.</p>
  ...
  /* ---------- units: the rules both editors follow ----------
     Livery Ledger's editor (colours and painting) and War Ledger's (datasheet, points and wargear) edit the
  ...
  function gearPicker(box, {kind, options, value, onChange}){
    ...
    draw();
    // fill: the weapons to start with, used when none are chosen yet or when replace is true.
    return {get value(){ return picked.join(", "); }, setOptions(o, fill, replace){ opts = o || []; if(fill && (replace || !picked.length)) picked = fill.slice(); draw(); }};
  }
  ...
  function openUnit(army, u, done, opts){
    ...
    const d = modal(u ? esc(u.name) : "Add a unit", `
      ...
      <div class="wgrid">
        <label class="span2">Datasheet<select id="w-sheet"><option value="">Custom unit (not in the list)</option>${u && u.datasheet && !sheetByName(army.faction, u.datasheet) ? `<option selected>${esc(u.datasheet)}</option>` : ""}${sheetOptions(army.faction, u ? u.datasheet : "")}</select></label>
        ...
        <label>Models<input id="w-count" type="number" min="1" max="99" inputmode="numeric" value="${u ? modelsOf(u) : 1}"></label>
    ...
    let ptsTouched = !!u;
    const sheetNow = () => { const n = v("w-sheet"); return sheetByName(sheetFaction(army.faction, n), n); };
    const sh0 = sheetNow();
    ...
    const gearChanged = () => { gearTouched = true; drawDs(); };
    const rangedPick = gearPicker($("w-ranged"), {kind: "ranged weapon", options: sh0 ? sh0.wr : [], value: u ? u.ranged : "", onChange: gearChanged});
    const meleePick = gearPicker($("w-melee"), {kind: "melee weapon", options: sh0 ? sh0.wm : [], value: u ? u.melee : "", onChange: gearChanged});
    ...
    $("w-sheet").addEventListener("change", () => {
      ...
      if(!u){ $("w-count").value = def.count;
        rangedPick.setOptions(sh.wr, (sh.wr || []).slice(0, 4), !gearTouched); meleePick.setOptions(sh.wm, (sh.wm || []).slice(0, 3), !gearTouched); }
    ...
    d.querySelector("form").addEventListener("submit", async e => {
      ...
      try {
        ...
        const ownFaction = sheetFaction(army.faction, row.datasheet);
        ...
        d.close(); flash(u ? (target !== army.id ? `Moved ${name}` : `Saved ${name}`) : `Added ${name}`); done();
      }
      catch(err){ console.error(err); $("w-msg").textContent = "Couldn't save: " + errText(err); b.disabled = false; }
    });
    if(u) armButton($("w-del"), "Press again to delete", async () => {
  ...
  async function viewWarArmy(id){
    ...
    let D = await warData(), filter = "all", by = "role", sortBy = "name", selecting = false;
    ...
    async function reload(){ army = await store.getArmy(id) || army; D = await warData(); draw(); }
    ...
    async function batch(change, done){
      ...
      // A change that returns false skipped that unit (it needed no change); only errors count as failures.
      let ok = 0, failed = 0;
      for(const u of us){ try { if(await change(u) !== false) ok++; } catch(err){ console.error(err); failed++; } }
      await reload(); updateBar();
      flash(failed ? `${done(ok)} ${failed} couldn't be changed.` : done(ok));
    }
    ...
    function forceGroups(list, t){
      ...
      const groups = ROLE_ORDER.map(role => [role, sorted.filter(u => (ROLE_ORDER.includes(u.role) ? u.role : "Other") === role)]);
      return groups.filter(g => g[1].length).map(([title, us]) => { const gt = sumUp(us);
        return `<section class="ro-group wc-group" aria-label="${esc(title)}"><div class="ro-gh"><div><h3>${esc(title)}</h3><small>${esc([plural(us.length, "unit"), plural(gt.models, "model"), ptsText(gt.points)].join(" · "))}</small></div></div>${table(us, null)}</section>`; }).join("");
    }
    function draw(){
      ...
      app.innerHTML = `
        ...
        <section class="war-stats" aria-label="Army overview">
          ...
          <div class="wstat"><b>${num(t.models)}</b><span>Models</span><small>${plural(units.length, "unit")}</small></div>
        ...
        <section class="panel" aria-labelledby="fc-h"><h2 class="ph" id="fc-h">Force composition</h2>
          <ul class="comp">${roles.map(([r, us]) => `<li><b>${us.length}</b><span>${esc(r)}</span><small>${plural(sumUp(us).models, "model")}</small></li>`).join("")}</ul>
        </section>
        <section class="war-sec" aria-labelledby="cf-h">
          ...
          <div class="list-tools tools-fill wa-tools">
            ...
            <div class="seg" role="group" aria-label="Show"><button type="button" data-filter="all" aria-pressed="${filter === "all"}">All units</button><button type="button" data-filter="fav" aria-pressed="${filter === "fav"}">${STAR(true)} Starred</button></div>
          ...
          ${shown.length ? forceGroups(shown, t) : `<p class="hint">No starred units yet. Tap the star beside a unit to keep it handy.</p>`}
        </section>` : `<section class="panel war-empty"><h2>No units yet</h2><p class="sub">Add units one at a time, whether you own them or not, or paste an army list to add them all at once.</p><div class="war-actions"><button type="button" class="primary" data-add-unit>+ Add unit</button><button type="button" data-from-list>Add from a list</button></div></section>`}
        <section class="war-sec" aria-labelledby="al-h"><div class="sec-h"><h2 id="al-h">Army lists</h2>${D.warMissing ? "" : `<button type="button" class="btn-sm" data-new-list="${esc(id)}">+ New list</button>`}</div>
          ${ls.length ? `<div class="ledgers">${ls.map(l => listCard(l, D)).join("")}</div>` : `<p class="hint">Build lists from this army for the games you play, and check each one against the points limit.</p>`}</section>
  ...
  function openCollectionAdd(D, done){
    const fav = (D.armies[0] || D.pools[0] || {}).faction || "";
    ...
    const fillArmies = () => {
      ...
      $("w-ca").innerHTML = `<option value="">Not in an army</option>${same.map(a => `<option value="${esc(a.id)}">${esc(a.name)}${a.faction !== fid ? ` (${esc(factionName(a.faction))})` : ""}</option>`).join("")}`;
    };
  ...
  // The detachments a pasted list names (a share code names one).
  const parsedDets = p => p.detachments || (p.detachment ? [p.detachment] : []);
  function exportText(list, s, army){
    ...
    // Datasheet names, as New Recruit writes them, so a unit you've renamed still reads back in.
    const nm = rowSheetName;
    ...
    const rest = rows.filter(x => !isCharRole(x.role) && x.role !== "Battleline").sort((a, b) => nm(a).localeCompare(nm(b)));
    const order = [...chars, ...line, ...rest];
    // Units that share a name get [1], [2]… so "Leading" and "Attached to" say which one.
    const tag = x => { const same = order.filter(y => nm(y) === nm(x)); return same.length > 1 ? `${nm(x)}[${same.indexOf(x) + 1}]` : nm(x); };
    ...
    const unitLine = (x, pre) => { const bits = [x.warlord ? "Warlord" : "", gear(x), x.enh ? x.enh.n : ""].filter(Boolean).join(", "); return `${pre}${x.count}x ${nm(x)} (${x.points} pts)${bits ? ": " + bits : ""}`; };
    ...
    const head = [bar, `+ FACTION KEYWORD: ${keyword}`, detText(list) ? `+ DETACHMENT: ${detText(list)}` : "", `+ TOTAL ARMY POINTS: ${s.points}pts`, "+",
      wl ? `+ WARLORD: ${charNo(wl)}${nm(wl)}` : "", `+ NUMBER OF UNITS: ${rows.length}`, bar].filter(Boolean);
    const out = [...head, ""];
    chars.forEach(x => {
  ...
  const dataBuilt = () => DATA.built || "";

  /* ---------- Crusade ---------- */
  // A Crusade force is an army list kept as an Order of Battle: its points limit is the supply limit, and
  // battles logged with it build up each unit's experience and the force's requisition points.
  const CR_RANKS = [[0, "Battle-ready"], [6, "Blooded"], [16, "Battle-hardened"], [31, "Heroic"], [51, "Legendary"]];
  const CR_HON = {trait: "Battle trait", weapon: "Weapon enhancement", relic: "Crusade relic", other: "Other honour"};
  const isCrusade = l => !!l && l.kind === "crusade";
  const crTag = l => isCrusade(l) ? `<span class="tag cr">Crusade</span>` : "";
  const victories = n => `${n} ${n === 1 ? "victory" : "victories"}`;
  // The force's battles, its requisition points (1 per battle plus any changed by hand) and each unit's share.
  function crusadeState(l, D){
    const gs = D.games.filter(g => g.listId === l.id);
    return {gs, rec: recordOf(gs), rp: Math.max(0, (l.rp || 0) + gs.length),
      per: k => ({battles: gs.filter(g => (g.took || []).includes(k)).length, mfg: gs.filter(g => g.mfg === k).length})};
  }
  // A unit's experience (1 per battle it took part in, 3 each time it was Marked for Greatness, plus any added
  // by hand), its rank, and its crusade points (1 per battle honour, less 1 per battle scar, plus any adjustment).
  // Only characters usually rise past Battle-hardened.
  function crUnit(x, cs){
    const p = cs.per(x.k), gained = p.battles + 3 * p.mfg, xp = Math.max(0, (x.e.xp || 0) + gained);
    const cap = isCharRole(x.role) ? CR_RANKS.length : 3, idx = Math.min(cap, CR_RANKS.filter(r => xp >= r[0]).length) - 1;
    const hon = x.e.hon || [], scar = x.e.scar || [];
    return {...p, gained, xp, idx, rank: CR_RANKS[idx][1], from: CR_RANKS[idx][0], next: idx + 1 < cap ? CR_RANKS[idx + 1][0] : null,
      hon, scar, cp: hon.length - scar.length + (x.e.cpx || 0)};
  }
  // The datasheet a list row is from: a collection unit's datasheet, or a list-only entry's sheet.
  const rowSheetName = x => x.u ? x.u.datasheet || x.name : x.sheet || x.name;
  // Where a list's points differ from the latest datasheet points: each unit whose points follow its
  // datasheet (not ones given their own points in this list, or a size outside the datasheet's range) and
  // each enhancement from the list's detachments. next is what the list would come to.
  function latestChanges(l, s, faction){
    const opts = enhOptions(faction, l.detachments), items = [];
    s.rows.filter(x => !x.gone).forEach(x => {
      const sh = sheetByName(faction, rowSheetName(x));
      if(sh && !(x.u && x.e.pts != null) && !(sh.ms && (x.count < sh.ms[0] || x.count > sh.ms[1])) && (sh.ms || sh.pb || x.count === 1)){
        const want = sheetPts(sh, x.count);
        if(want != null && want !== x.base) items.push({kind: x.u ? "unit" : "entry", x, name: x.name + (x.count > 1 ? ` (${x.count} models)` : ""), from: x.base, to: want});
      }
      const o = x.enh && opts.find(e => e[0].toLowerCase() === x.enh.n.toLowerCase());
      if(o && x.enh.p !== o[1]) items.push({kind: "enh", x, name: `${o[0]} on ${x.name}`, from: x.enh.p, to: o[1]});
    });
    return {items, next: s.points + items.reduce((a, c) => a + c.to - c.from, 0), shared: items.some(c => c.kind === "unit"),
      sig: items.map(c => `${c.x.k}.${c.kind}.${c.to}`).join(",").slice(0, 100)};
  }
  const DAY_MS = 864e5;
  function listChecks(l, s, faction){
    const out = [], add = (id, text, more, extra) => out.push({id, text, more, ...extra});
    const rows = s.rows.filter(x => !x.gone), sheets = sheetsOf(faction);
    const sheetOf = x => sheetByName(faction, rowSheetName(x));
    const epic = x => x.role === "Epic Hero" || !!(sheetOf(x) || {}).eh;
    if(!rows.length) return out;
    if(l.limit && s.points > l.limit) add("over", `${num(s.points - l.limit)} pts over the ${isCrusade(l) ? "supply " : ""}limit.`, `The ${isCrusade(l) ? "Order of Battle" : "list"} comes to ${ptsText(s.points)} against a limit of ${ptsText(l.limit)}.`);
    ...
    const bySheet = {};
    rows.forEach(x => { const n = rowSheetName(x); (bySheet[n] = bySheet[n] || []).push(x); });
    ...
    enh.forEach(x => {
      ...
      if(o[2] && o[2].every(n => names.has(n)) && !o[2].includes(rowSheetName(x))) add(`enh-only:${x.k}:${o[0]}`, `${o[0]} is usually for ${o[2].join(" or ")} only.`, `${x.name} has it in this list.`);
    });
    rows.forEach(x => {
    ...
    rows.forEach(x => {
      const sh = sheetOf(x);
      if(sh && sh.ms && (x.count < sh.ms[0] || x.count > sh.ms[1])) add(`size:${x.k}:${x.count}`, `${x.name} has ${plural(x.count, "model")}.`, `The datasheet's unit size is ${sh.ms[0] === sh.ms[1] ? sh.ms[0] : `${sh.ms[0]} to ${sh.ms[1]}`} models.`);
    });
    // Points that differ from the latest datasheet points, as one note with a button to update them all.
    const ch = latestChanges(l, s, faction);
    if(ch.items.length){
      const over = l.limit && ch.next > l.limit;
      add("latest:" + ch.sig, `The latest points change this list from ${ptsText(s.points)} to ${ptsText(ch.next)}${over ? `, ${num(ch.next - l.limit)} over the limit` : ""}.`,
        `${dataBuilt() ? `Datasheet points were updated ${dayText(dataBuilt())}. ` : ""}${ch.shared ? "Updating also changes the points on the units in your collection, so other lists using them change too." : ""}`,
        {items: ch.items.map(c => `${c.name}: ${num(c.from)} → ${num(c.to)} pts`), act: "Update to latest points"});
    }
    if(l.ptsAsOf){
      const days = Math.floor((Date.now() - new Date(l.ptsAsOf + "T12:00:00")) / DAY_MS);
      if(days > 90) add("date:" + l.ptsAsOf, `Points last checked ${dayText(l.ptsAsOf)}.`, "Points change a few times a year, so check they're still current.");
    ...
    // A Crusade force's Order of Battle isn't a list for one game, so only its size and points matter here.
    return isCrusade(l) ? out.filter(c => /^(over|size|latest)/.test(c.id)) : out;
  }
  // What a list holds: each entry's unit (or the datasheet it was pasted as), models and points.
  ...
  function listState(l, D){
    ...
    const rows = l.units.map((e, i) => {
      ...
      if(e.u){
        ...
        return {i, ...extra, u, name: u.name, role: u.role, count: modelsOf(u), base, points: base + ep};
      }
  ...
  function listCard(l, D){
    ...
    const note = !s.rows.length ? "No units yet" : checks ? `<span class="tc-chip">${plural(checks, "thing")} to check</span>` : plural(s.models, "model");
    return `<a class="lcard war-card${l.status === "archived" ? " archived" : ""}" href="#/war/list/${esc(l.id)}">
      <div class="card-top">${a ? armyBadge(a, 44) : ""}<div><h3>${esc(l.name)}</h3><div class="meta">${esc([a ? a.name : "Deleted army", sizeName(l), detText(l)].filter(Boolean).join(" · "))}</div></div><span class="tags">${crTag(l)}${statusTag(l)}</span></div>
      <div class="wc-nums"><span><b>${num(s.points)}</b>${l.limit ? ` / ${num(l.limit)}` : ""} ${isCrusade(l) ? "supply" : "pts"}</span><span><b>${s.rows.length}</b> ${s.rows.length === 1 ? "unit" : "units"}</span>${isCrusade(l) ? `<span><b>${crusadeState(l, D).rp}</b> RP</span>` : ""}${gs.length ? `<span><b>${recText(r)}</b> record</span>` : ""}</div>
      ${l.limit ? `<div class="prog" aria-hidden="true"><i style="width:${Math.min(100, pctOf(s.points, l.limit))}%"></i></div>` : ""}
      <div class="foot"><span>${l.limit ? (s.points > l.limit ? `${num(s.points - l.limit)} over the limit` : `${num(l.limit - s.points)} left`) : "No points limit"}</span><span>${note}</span></div>
    </a>`;
  }
  ...
  function latestBanner(D){
    ...
    const hit = D.lists.filter(l => l.status !== "archived").map(l => {
      const a = D.armies.find(x => x.id === l.armyId); if(!a) return null;
      const s = listState(l, D), ch = latestChanges(l, s, a.faction);
      return ch.items.length && !(l.ignored || []).includes("latest:" + ch.sig) ? [l, s.points, ch.next] : null;
    }).filter(Boolean);
  ...
  function listFields(l, D, faction){
    ...
    return `
      ...
      <p class="hint">Points change when Games Workshop updates them, so note when you last checked this list's.${built ? ` Datasheet points in War Ledger were last updated ${esc(dayText(built))}.` : ""}</p>`;
  }
  ...
  function openNewList(D, armyId, kind){
    ...
    wireListFields(d, fac); wireKind(d);
    // Detachment suggestions follow the army's faction.
    $("w-la").addEventListener("change", () => {
      const keep = readListFields(d);
      $("w-lf").innerHTML = listFields(keep, D, fac()); wireListFields(d, fac);
    });
    d.querySelector("form").addEventListener("submit", async e => {
      e.preventDefault();
      const f = readListFields(d), kind = $("w-lk").value, name = $("w-ln").value.trim() || (kind === "crusade" ? "Crusade force" : f.limit ? `${num(f.limit)} points` : "Army list");
      try { const l = await store.saveList({armyId: $("w-la").value, name, ...f, kind, units: []}); d.close(); location.hash = `#/war/list/${l.id}`; }
  ...
  async function viewWarList(id){
    let D = await warData(), list = D.lists.find(l => l.id === id);
    ...
    async function reload(){ D = await warData(); list = D.lists.find(l => l.id === id) || list; army = D.armies.find(a => a.id === list.armyId) || army; draw(); }
    ...
    /* The builder, New Recruit style: pick the battle size and detachment, add units straight from the
       datasheets, and set each unit's size with its points worked out from the datasheet. */
    const roleOf = sh => ROLE_ORDER.includes(sh.r) ? sh.r : "Other";
    const sizesOf = sh => sh && sh.ms && sh.ms[1] > sh.ms[0] ? Array.from({length: sh.ms[1] - sh.ms[0] + 1}, (_, j) => sh.ms[0] + j) : [];
    const entrySheet = x => sheetByName(army.faction, x.u ? x.u.datasheet : x.sheet || x.name);
    function sizePick(x){
      const sh = !x.u && entrySheet(x), sizes = sizesOf(sh);
      if(!sizes.length) return `<small>${plural(x.count, "model")}</small>`;
      if(!sizes.includes(x.count)) sizes.push(x.count);
      return `<label class="lb-size"><span class="sr-only">Models in ${esc(x.name)}</span><select data-size="${x.i}">${sizes.sort((a, b) => a - b).map(n => `<option value="${n}"${n === x.count ? " selected" : ""}>${plural(n, "model")}${sheetPts(sh, n) != null ? ` · ${sheetPts(sh, n)} pts` : ""}</option>`).join("")}</select></label>`;
    }
    // Battle size and detachment, right in the roster.
    function configRow(){
    ...
    function sheetRows(s){
      const inList = {};
      s.rows.filter(x => !x.gone).forEach(x => { const n = rowSheetName(x); inList[n] = (inList[n] || 0) + 1; });
      const sheets = sheetsOf(army.faction).filter(sh => (dLegends || !sh.t) && (!dRole || roleOf(sh) === dRole) && (!dq || sh.n.toLowerCase().includes(dq)));
    ...
    function draw(){
      ...
      app.innerHTML = `
        ...
        <section class="war-stats" aria-label="List summary">
          <div class="wstat${over ? " over" : ""}"><b>${num(s.points)}${list.limit ? `<small> / ${num(list.limit)}</small>` : ""}</b><span>${isCrusade(list) ? "Supply used" : "Points"}</span>${list.limit ? `<div class="wbar" aria-hidden="true"><i style="width:${Math.min(100, pctOf(s.points, list.limit))}%"></i></div><small>${over ? `${num(s.points - list.limit)} over the limit` : `${num(list.limit - s.points)} left`}</small>` : ""}</div>
      ...
      const dlist = () => { $("lb-dlist").innerHTML = sheetRows(listState(list, D)); };
      if($("lb-dq")){
        $("lb-dq").addEventListener("input", e => { dq = e.target.value.trim().toLowerCase(); dlist(); });
        $("lb-dr").addEventListener("change", e => { dRole = e.target.value; dlist(); });
        $("lb-dl").addEventListener("change", e => { dLegends = e.target.checked; dlist(); });
      }
      $("lb-bs").addEventListener("change", e => {
        const v = e.target.value, z = BATTLE_SIZES.find(b => b.id === v);
        save({size: v, limit: z ? z.pts : v === "custom" ? list.limit : 0});
      });
      if($("lb-lim")) $("lb-lim").addEventListener("change", e => save({limit: Math.max(0, parseInt(e.target.value, 10) || 0)}));
      if($("lb-det")) $("lb-det").addEventListener("change", e => save({detachments: e.target.value ? [e.target.value] : []}));
      app.querySelectorAll("[data-size]").forEach(sel => sel.addEventListener("change", () => {
        const i = +sel.dataset.size, e = list.units[i], sh = sheetByName(army.faction, e.sheet || e.n), n = +sel.value, p = sheetPts(sh, n);
        save({units: list.units.map((x, j) => j === i ? {...x, count: n, ...(p != null ? {points: p} : {})} : x)});
        const again = app.querySelector(`[data-size="${i}"]`); if(again) again.focus();
      }));
        ...
        <section class="panel cr-ob" aria-labelledby="ob-h"><h2 class="ph" id="ob-h">Order of Battle</h2>
          ...
          ${us.length ? `<div class="wt-scroll"><table class="wtable wt-cards cr-t">
            ...
            <tbody>${us.map(([x, c]) => `<tr><th scope="row"><button type="button" class="linkish" data-cr="${x.i}">${esc(x.name)}</button>${c.hon.length || c.scar.length ? `<small>${esc([...c.hon.map(h => h.n), ...c.scar.map(n => "Scar: " + n)].join(" · "))}</small>` : ""}</th>
              ...
              <td class="n" data-label="Honours">${c.hon.length}</td><td class="n" data-label="Scars">${c.scar.length}</td><td class="n" data-label="Crusade pts">${c.cp}</td></tr>`).join("")}</tbody>
          </table></div>` : `<p class="hint">Add units from your collection below to start your Order of Battle.</p>`}
        </section>`;
    }
    // One unit's Crusade card: experience, battle honours, battle scars, crusade points and notes.
    function openCrusadeUnit(i){
      const s = listState(list, D), x = s.rows.find(r => r.i === i); if(!x || x.gone) return;
      const cs = crusadeState(list, D), c = crUnit(x, cs), e = list.units[i];
      let hon = c.hon.slice(), scar = c.scar.slice();
      const d = modal(`${esc(x.name)}: Crusade card`, `
        <p class="sub cr-now"><span class="cr-rank r${c.idx}">${esc(c.rank)}</span> ${c.next != null ? `${c.next - c.xp} XP to ${esc(CR_RANKS[c.idx + 1][1])}` : isCharRole(x.role) ? "The highest rank" : "Only characters usually rise past Battle-hardened"}</p>
        <div class="wgrid">
          <label>Experience (XP)<input id="cr-xp" type="number" min="0" max="999" inputmode="numeric" value="${c.xp}"></label>
          <label class="span2">Crusade points adjustment<input id="cr-cpx" type="number" min="-99" max="99" inputmode="numeric" value="${e.cpx || ""}" placeholder="0"></label>
        </div>
        <p class="hint">${c.gained ? `${c.gained} XP came from ${plural(c.battles, "battle")}${c.mfg ? ` and ${c.mfg}× Marked for Greatness` : ""}. ` : ""}Change the total for kills, agendas or anything else that gives experience. Crusade points are 1 per battle honour, less 1 per scar; adjust for honours worth more.</p>
        <fieldset class="wfs"><legend>Battle honours</legend><ul class="cr-items" id="cr-hon"></ul>
          <div class="cr-add"><select id="cr-ht" aria-label="Kind of honour">${Object.entries(CR_HON).map(([k, v]) => `<option value="${k}">${v}</option>`).join("")}</select><input id="cr-hn" maxlength="80" placeholder="e.g. Hardened veterans" aria-label="Battle honour"><button type="button" class="btn-sm" id="cr-hadd">Add</button></div></fieldset>
        <fieldset class="wfs"><legend>Battle scars</legend><ul class="cr-items" id="cr-scar"></ul>
          <div class="cr-add"><input id="cr-sn" maxlength="80" placeholder="e.g. Battle-weary" aria-label="Battle scar"><button type="button" class="btn-sm" id="cr-sadd">Add</button></div></fieldset>
        <label>Notes<textarea id="cr-note" rows="3" maxlength="400" placeholder="Kills, deeds and grudges">${esc(e.cnote || "")}</textarea></label>
        <div class="row-actions"><button type="submit" class="primary">Save</button></div>`, "wide");
      const paint = () => {
        $("cr-hon").innerHTML = hon.length ? hon.map((h, j) => `<li><span>${esc(h.n)} <small>${esc(CR_HON[h.t] || CR_HON.other)}</small></span><button type="button" class="btn-sm icon-x" data-rmh="${j}" aria-label="Remove ${esc(h.n)}">×</button></li>`).join("") : `<li class="hint">None yet.</li>`;
        $("cr-scar").innerHTML = scar.length ? scar.map((n, j) => `<li><span>${esc(n)}</span><button type="button" class="btn-sm icon-x" data-rms="${j}" aria-label="Remove ${esc(n)}">×</button></li>`).join("") : `<li class="hint">None.</li>`;
      };
      paint();
      const addHon = () => { const n = $("cr-hn").value.trim(); if(!n || hon.length >= 20) return; hon.push({t: $("cr-ht").value, n}); $("cr-hn").value = ""; paint(); $("cr-hn").focus(); };
      const addScar = () => { const n = $("cr-sn").value.trim(); if(!n || scar.length >= 20) return; scar.push(n); $("cr-sn").value = ""; paint(); $("cr-sn").focus(); };
      $("cr-hadd").addEventListener("click", addHon); $("cr-sadd").addEventListener("click", addScar);
      // Enter in an add box adds the item instead of saving the card.
      $("cr-hn").addEventListener("keydown", ev => { if(ev.key === "Enter"){ ev.preventDefault(); addHon(); } });
      $("cr-sn").addEventListener("keydown", ev => { if(ev.key === "Enter"){ ev.preventDefault(); addScar(); } });
      d.addEventListener("click", ev => {
        const h = ev.target.closest("[data-rmh]"), sc = ev.target.closest("[data-rms]");
        if(h){ hon.splice(+h.dataset.rmh, 1); paint(); } else if(sc){ scar.splice(+sc.dataset.rms, 1); paint(); }
      });
      d.querySelector("form").addEventListener("submit", ev => {
        ev.preventDefault();
        const n = {...e, xp: Math.max(0, parseInt($("cr-xp").value, 10) || 0) - c.gained, hon, scar, cpx: parseInt($("cr-cpx").value, 10) || 0, cnote: $("cr-note").value.trim()};
        save({units: list.units.map((u, j) => j === i ? n : u)}); d.close();
      });
    }
    function openRequisition(){
      const cs = crusadeState(list, D);
      const d = modal("Requisition points", `
        <label>Requisition points now<input id="cr-rp" type="number" min="0" max="999" inputmode="numeric" value="${cs.rp}"></label>
        <p class="hint">The force gains 1 for each battle logged with it (${cs.gs.length} so far). Lower this when you spend them on requisitions, or raise it when a rule gives you more.</p>
    ...
    onApp(async e => {
      ...
      if(b.dataset.add) save({units: list.units.concat({u: b.dataset.add, k: entryKey()})});
        ...
        // A unit with a Crusade record takes a second press, since its experience and honours go with it.
        if(isCrusade(list) && gone && (gone.xp || gone.hon || gone.scar || D.games.some(g => g.listId === list.id && (g.took || []).includes(gone.k))) && rmArmed !== gone.k){
          rmArmed = gone.k; clearTimeout(rmTimer); rmTimer = setTimeout(() => { rmArmed = ""; }, 5000);
          flash("Press × again to remove it. Its experience, honours and scars will be lost."); return;
        }
        rmArmed = "";
      ...
      else if(b.dataset.cr != null) openCrusadeUnit(+b.dataset.cr);
      else if(b.matches("[data-rp]")) openRequisition();
      else if(b.matches("[data-latest]")) {
        // Units from the collection take the new points on the unit itself; the rest change in this list.
        ...
        } catch(err){ console.error(err); flash("Couldn't update the points: " + errText(err)); b.disabled = false; }
      }
        ...
        const d = modal("List details", `
          <label>Name<input id="w-ln" maxlength="80" value="${esc(list.name)}"></label>
          ${kindField(list.kind)}
          ${listFields(list, D, army.faction)}
          <label>Notes<textarea id="w-lnote" rows="3" maxlength="600">${esc(list.notes)}</textarea></label>
          <div class="row-actions"><button type="submit" class="primary">Save</button></div>`, "wide");
        wireListFields(d, army.faction); wireKind(d);
        d.querySelector("form").addEventListener("submit", ev => {
          ev.preventDefault();
          save({name: $("w-ln").value.trim() || list.name, kind: $("w-lk").value, ...readListFields(d), notes: $("w-lnote").value.trim()});
        ...
        let copy = {...list};
        if(isCrusade(list)){ const cs = crusadeState(list, D), rows = listState(list, D).rows; copy = {...copy, rp: cs.rp, units: list.units.map((e, i) => { const x = rows.find(r => r.i === i); return x && !x.gone ? {...e, xp: crUnit(x, cs).xp} : e; })}; }
        try { await chain; const c = await store.saveList({...copy, name: nextVersion(list.name, D.lists), status: "draft", from: list.id, ignored: []}); flash("List duplicated. Compare it with the original at the bottom of the page."); location.hash = `#/war/list/${c.id}`; }
        catch(err){ flash("Couldn't duplicate: " + errText(err)); }
      }
      else if(b.matches("[data-compare]")) {
        const other = compareDefault(list, D);
        // The older version goes first, so the page reads as what changed.
        if(other) location.hash = other.id === list.from ? `#/war/compare/${other.id}/${list.id}` : `#/war/compare/${list.id}/${other.id}`; else flash("Make another list first, for example with Duplicate list, then compare the two.");
      }
      else warClicks(e, D, reload);
    });
  }
  // "Club night" → "Club night v2", "Club night v2" → "Club night v3", skipping names already taken.
  function nextVersion(name, lists){
    const m = name.match(/^(.*?)\s+v(\d+)$/i), base = m ? m[1] : name.replace(/\s+\(copy\)$/i, "");
    let n = m ? +m[2] + 1 : 2;
    while(lists.some(l => l.name.toLowerCase() === `${base} v${n}`.toLowerCase())) n++;
    return `${base} v${n}`.slice(0, 80);
  }
  // Who to compare a list with first: the list it was copied from, a copy of it, or the newest other list of the faction.
  function compareDefault(l, D){
    const fac = id => (D.armies.find(a => a.id === id) || {}).faction, others = D.lists.filter(x => x.id !== l.id);
    return others.find(x => x.id === l.from) || others.find(x => x.from === l.id)
      || others.filter(x => fac(x.armyId) === fac(l.armyId)).sort((a, b) => (b.updatedAt || "").localeCompare(a.updatedAt || ""))[0] || others[0] || null;
  }
  // Two lists' units matched up: the same collection unit first, then the same datasheet.
  function diffLists(sa, sb){
    const A = sa.rows.filter(x => !x.gone), B = sb.rows.filter(x => !x.gone), used = new Set(), pairs = [], onlyA = [];
    const sheet = x => rowSheetName(x).toLowerCase();
    const take = (x, test) => { const y = B.find(y => !used.has(y) && test(y)); if(y){ used.add(y); pairs.push([x, y]); } return !!y; };
    A.filter(x => !(x.u && take(x, y => y.u && y.u.id === x.u.id))).forEach(x => { if(!take(x, y => sheet(y) === sheet(x) && y.count === x.count) && !take(x, y => sheet(y) === sheet(x))) onlyA.push(x); });
    const lead = (x, s) => x.lead ? (s.rows.find(r => r.k === x.lead && !r.gone) || {}).name || "" : "";
    const changed = [], same = [];
    pairs.forEach(([x, y]) => {
      const d = [];
      if(x.count !== y.count) d.push(`${plural(x.count, "model")} → ${plural(y.count, "model")}`);
      if(x.points !== y.points) d.push(`${num(x.points)} → ${num(y.points)} pts`);
      const en = e => e ? e.n : "";
      if(en(x.enh) !== en(y.enh)) d.push(`Enhancement: ${en(x.enh) || "none"} → ${en(y.enh) || "none"}`);
      if(x.warlord !== y.warlord) d.push(y.warlord ? "Now the warlord" : "No longer the warlord");
      if(lead(x, sa) !== lead(y, sb)) d.push(`Leading: ${lead(x, sa) || "nobody"} → ${lead(y, sb) || "nobody"}`);
      (d.length ? changed : same).push({x, y, d});
    });
    return {onlyA, onlyB: B.filter(y => !used.has(y)), changed, same};
  }
  async function viewWarCompare(aId, bId){
    view.name = "war-compare"; document.title = "Compare lists · War Ledger";
    const D = await warData();
    let a = D.lists.find(l => l.id === aId), b = D.lists.find(l => l.id === bId);
    if(a && (!b || b.id === a.id)) b = compareDefault(a, D);
    if(!a || !b){
      app.innerHTML = `${warTabs("lists")}<div class="banner"><span class="dot warn"></span><span>${D.lists.length < 2 ? "You need two army lists to compare. Duplicate a list to make a new version of it." : "Those lists couldn't be found. They may have been deleted."}</span></div><p><a class="btn" href="#/war/lists">Your army lists</a></p>`;
      return;
    }
    const sa = listState(a, D), sb = listState(b, D), df = diffLists(sa, sb), gA = D.games.filter(g => g.listId === a.id), gB = D.games.filter(g => g.listId === b.id);
    const armyName = l => (D.armies.find(x => x.id === l.armyId) || {}).name || "Deleted army";
    const nChecks = (l, s) => { const ar = D.armies.find(x => x.id === l.armyId); if(!ar || !listChecksOn()) return null; const hid = new Set(l.ignored || []); return listChecks(l, s, ar.faction).filter(c => !hid.has(c.id)).length; };
    const opt = sel => D.lists.map(l => `<option value="${esc(l.id)}"${l.id === sel ? " selected" : ""}>${esc(l.name)} (${esc(armyName(l))})</option>`).join("");
    const row = (label, va, vb) => `<tr${va === vb ? "" : ` class="cmp-diff"`}><th scope="row">${label}</th><td>${va}</td><td>${vb}</td></tr>`;
    const ca = nChecks(a, sa), cb = nChecks(b, sb);
    const unitLi = x => `<li><span>${esc(x.name)}${x.count > 1 ? ` <small>${plural(x.count, "model")}</small>` : ""}${x.warlord ? ` <span class="tag wl">Warlord</span>` : ""}</span><span class="lb-pts">${num(x.points)}</span></li>`;
    const sum = xs => ptsText(xs.reduce((t, x) => t + x.points, 0));
    app.innerHTML = `
      <div class="crumbs"><a href="#/war">War Ledger</a> / <a href="#/war/lists">Army lists</a> / Compare</div>
      <section class="page-head war-head"><div><p class="eyebrow">War Ledger</p><h1>Compare lists</h1><p class="sub">What changes between two lists: units, points, detachments and how each has done.</p></div></section>
      ${warTabs("lists")}
      <div class="cmp-pick">
        <label>First list<select id="cmp-a">${opt(a.id)}</select></label>
        <button type="button" class="btn-sm" id="cmp-swap" aria-label="Swap the two lists">⇄</button>
        <label>Second list<select id="cmp-b">${opt(b.id)}</select></label>
      </div>
      <section class="panel cmp-sum" aria-label="Side by side"><div class="wt-scroll"><table class="wtable compact cmp-t">
        <thead><tr><th scope="col"><span class="sr-only">Detail</span></th><th scope="col"><a href="#/war/list/${esc(a.id)}">${esc(a.name)}</a></th><th scope="col"><a href="#/war/list/${esc(b.id)}">${esc(b.name)}</a></th></tr></thead>
        <tbody>
          ${row("Points", `${num(sa.points)}${a.limit ? ` / ${num(a.limit)}` : ""}`, `${num(sb.points)}${b.limit ? ` / ${num(b.limit)}` : ""}`)}
          ${row("Battle size", esc(sizeName(a) || "—"), esc(sizeName(b) || "—"))}
          ${row("Detachments", esc(detText(a) || "—"), esc(detText(b) || "—"))}
          ${row("Units", `${sa.rows.filter(x => !x.gone).length}`, `${sb.rows.filter(x => !x.gone).length}`)}
          ${row("Models", `${sa.models}`, `${sb.models}`)}
          ${ca == null ? "" : row("Things to check", `${ca}`, `${cb}`)}
          ${row("Record", gA.length ? `${recText(recordOf(gA))} <small>(${plural(gA.length, "game")})</small>` : "No games", gB.length ? `${recText(recordOf(gB))} <small>(${plural(gB.length, "game")})</small>` : "No games")}
          ${row("Status", esc(statusName(a.status)), esc(statusName(b.status)))}
        </tbody></table></div></section>
      <div class="cmp-grid">
        <section class="panel" aria-labelledby="cmp-oa"><h2 class="ph" id="cmp-oa">Only in ${esc(a.name)} <small>${sum(df.onlyA)}</small></h2>
          ${df.onlyA.length ? `<ul class="lb-rows cmp-rows">${df.onlyA.map(unitLi).join("")}</ul>` : `<p class="hint">Nothing. Every unit is also in ${esc(b.name)}.</p>`}</section>
        <section class="panel" aria-labelledby="cmp-ob"><h2 class="ph" id="cmp-ob">Only in ${esc(b.name)} <small>${sum(df.onlyB)}</small></h2>
          ${df.onlyB.length ? `<ul class="lb-rows cmp-rows">${df.onlyB.map(unitLi).join("")}</ul>` : `<p class="hint">Nothing. Every unit is also in ${esc(a.name)}.</p>`}</section>
      </div>
      <section class="panel" aria-labelledby="cmp-ch"><h2 class="ph" id="cmp-ch">In both, but different</h2>
        ${df.changed.length ? `<ul class="lb-rows cmp-rows">${df.changed.map(c => `<li><span class="lb-name">${esc(c.x.name)}<small>${c.d.map(esc).join(" · ")}</small></span><span class="lb-pts">${c.y.points - c.x.points > 0 ? "+" : ""}${c.y.points - c.x.points ? num(c.y.points - c.x.points) : ""}</span></li>`).join("")}</ul>` : `<p class="hint">No units are set up differently.</p>`}
        <p class="hint">${df.same.length ? `${plural(df.same.length, "unit")} ${df.same.length === 1 ? "is" : "are"} the same in both: ${esc(df.same.map(c => c.x.name).join(", "))}.` : "No units are the same in both."}</p>
      </section>`;
    const go = () => { location.hash = `#/war/compare/${$("cmp-a").value}/${$("cmp-b").value}`; };
    $("cmp-a").addEventListener("change", go); $("cmp-b").addEventListener("change", go);
    $("cmp-swap").addEventListener("click", () => { location.hash = `#/war/compare/${b.id}/${a.id}`; });
  }
  ...
  function openListImport(army, list, D, apply){
    const d = modal("Paste an army list", `
      <p class="sub">Paste a list from the Warhammer 40,000 app, New Recruit, BattleScribe or a list-builder share code. Each unit is matched to one in your collection, or added from its datasheet.</p>
    ...
    const read = () => {
      ...
      $("w-found").innerHTML = `<p><strong>${plural(entries.length, "unit")}</strong> · ${ptsText(parsed.units.reduce((a, x) => a + x.points, 0))} · ${own} matched to your collection</p>
        <ul>${entries.map(x => `<li>${esc(x.name)} <small>${x.owned ? "In your collection" : "From the datasheet"}</small></li>`).join("")}</ul>${parsed.unmatched.length ? `<p class="hint">Not recognised: ${esc(parsed.unmatched.join(", "))}</p>` : ""}`;
    ...
    d.querySelector("form").addEventListener("submit", e => {
      ...
      // One warlord per list: a pasted warlord steps down the list's own.
      const incoming = plan.entries.map(x => x.e), kept = $("w-repl").checked ? [] : list.units;
      const units = (incoming.some(e => e.warlord) ? kept.map(u => u.warlord ? (({warlord, ...r}) => r)(u) : u) : kept).concat(incoming);
      const patch = {units};
      if(plan.parsed.limit && !list.limit) patch.limit = plan.parsed.limit;
      if(plan.parsed.detachment && !(list.detachments || []).length) patch.detachments = parsedDets(plan.parsed);
      if(patch.limit && !list.size) patch.size = sizeOf({limit: patch.limit});
  ...
  function openGame(D, g, again){
    ...
    const d = modal(edit ? "Edit battle" : "Log a battle", `
      ...
      <label id="w-gv-l"><span>Most valuable unit <span class="opt">(optional)</span></span><select id="w-gv"></select></label>
      <fieldset class="wfs" id="w-gcr" hidden></fieldset>
    ...
    // A Crusade force's battle: which units took part and which was Marked for Greatness (instead of the MVP).
    const crList = () => { const l = D.lists.find(x => x.id === $("w-gl").value); return isCrusade(l) ? l : null; };
    const crFill = () => {
      const l = crList(), box = $("w-gcr");
      $("w-gv-l").hidden = !!l; box.hidden = !l;
      if(!l){ box.innerHTML = ""; return; }
      const rows = listState(l, D).rows.filter(x => !x.gone), same = seed.listId === l.id && edit;
      const took = k => same ? (seed.took || []).includes(k) : true;
      box.innerHTML = `<legend>Crusade: ${esc(l.name)}</legend>
        <p class="hint">Units that took part gain 1 XP, and the one Marked for Greatness 3 more. The force gains 1 requisition point.</p>
        <div class="cr-took">${rows.map(x => `<label class="chk"><input type="checkbox" name="w-took" value="${esc(x.k)}"${took(x.k) ? " checked" : ""}><span>${esc(x.name)}</span></label>`).join("")}</div>
    ...
    d.querySelector("form").addEventListener("submit", async e => {
      ...
      if(cl){
        ...
        // The unit Marked for Greatness counts as the battle's most valuable unit too.
        const m = cl.units.find(x => x.k === row.mfg); row.mvp = m && m.u ? m.u : "";
        if(row.mfg && !row.took.includes(row.mfg)) row.took.push(row.mfg);
      }
      const b = e.submitter || d.querySelector("[type=submit]"); b.disabled = true; $("w-msg").textContent = "Saving…";
    ...
    if(edit) armButton($("w-gdel"), "Press again to delete", async () => {
      try { await store.removeGame(g.id); await syncRecords([g.armyId]); d.close(); flash("Battle deleted."); again(); }
      catch(err){ $("w-msg").textContent = "Couldn't delete: " + errText(err); }
    });
  }
  // Streaks, scores and biggest win for a set of battles (newest first).
  function battleHighlights(gs){
    const cur = gs.length ? gs.findIndex(g => g.result !== gs[0].result) : 0, curN = cur < 0 ? gs.length : cur;
    let best = 0, run = 0;
    gs.slice().reverse().forEach(g => { run = g.result === "w" ? run + 1 : 0; best = Math.max(best, run); });
    const sc = gs.filter(g => g.us != null && g.them != null), avg = k => sc.length ? Math.round(sc.reduce((a, g) => a + g[k], 0) / sc.length) : 0;
    const big = sc.filter(g => g.us > g.them).sort((a, b) => (b.us - b.them) - (a.us - a.them))[0];
    const cell = (b, l, sm) => `<div class="wstat"><b>${b}</b><span>${l}</span>${sm ? `<small>${sm}</small>` : ""}</div>`;
    return `<section class="war-stats" aria-label="Highlights">
      ${cell(gs.length ? `${curN} ${RES[gs[0].result].toLowerCase()}${curN === 1 ? "" : gs[0].result === "l" ? "es" : "s"}` : "—", "Current streak", gs.length ? `Since ${esc(dayText(gs[Math.min(curN, gs.length) - 1].date))}` : "")}
      ${cell(best, "Best winning streak", best ? `${best === 1 ? "game" : "games"} in a row` : "No wins yet")}
      ${cell(sc.length ? `${avg("us")}<small> – ${avg("them")}</small>` : "—", "Average score", sc.length ? `Over ${plural(sc.length, "game")} with scores` : "Add scores when you log battles")}
      ${cell(big ? `+${big.us - big.them}` : "—", "Biggest win", big ? `${big.us}–${big.them} vs ${esc(oppText(big))}` : "")}
    </section>`;
  }
  // Each game's score margin (yours minus theirs), oldest on the left, up for ahead and down for behind.
  // Games without scores sit on the line. The history below is the same data as a list.
  function marginChart(gs, D){
    const pts = gs.slice(0, 30).reverse();
    if(pts.filter(g => g.us != null && g.them != null).length < 2) return "";
    const W = Math.round(Math.max(300, Math.min(900, (app.clientWidth || innerWidth) - 60))), H = 200, pad = 24, mid = H / 2, m = g => g.us != null && g.them != null ? g.us - g.them : null;
    const top = Math.max(10, ...pts.map(g => Math.abs(m(g) || 0))), step = Math.ceil(top / 10) * 10, k = (mid - pad) / step;
    const band = (W - 48) / pts.length, bw = Math.min(24, band - 2);
    const bars = pts.map((g, i) => {
      const v = m(g), x = 40 + i * band + (band - bw) / 2, h = v == null ? 0 : Math.max(2, Math.abs(v) * k), y = v == null || v >= 0 ? mid - h : mid;
      const tip = `${dayText(g.date)} · ${RES[g.result]} vs ${oppText(g)}${v == null ? " · no score" : ` · ${g.us}–${g.them}`}`;
      const r = Math.min(4, bw / 2, h), path = v == null ? `<circle cx="${x + bw / 2}" cy="${mid}" r="4" class="mc-${g.result}"/>`
        : v >= 0 ? `<path class="mc-${g.result}" d="M${x},${mid}V${y + r}Q${x},${y} ${x + r},${y}H${x + bw - r}Q${x + bw},${y} ${x + bw},${y + r}V${mid}Z"/>`
        : `<path class="mc-${g.result}" d="M${x},${mid}V${mid + h - r}Q${x},${mid + h} ${x + r},${mid + h}H${x + bw - r}Q${x + bw},${mid + h} ${x + bw},${mid + h - r}V${mid}Z"/>`;
      return `<g class="mc-bar" data-tip="${esc(tip)}"><rect x="${40 + i * band}" y="0" width="${band}" height="${H}" fill="transparent"/>${path}<title>${esc(tip)}</title></g>`;
    }).join("");
    const wins = pts.filter(g => g.result === "w").length;
    return `<section class="panel mchart" aria-labelledby="mc-h"><h2 class="ph" id="mc-h">Score margin</h2>
      <p class="hint">Your score minus your opponent's in your last ${plural(pts.length, "game")}, oldest first. Hover or tap a bar for the game.</p>
      <div class="mc-wrap"><svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Score margin in your last ${plural(pts.length, "game")}: ${wins} won. The history below lists every game.">
        <line x1="40" x2="${W - 8}" y1="${mid - step * k}" y2="${mid - step * k}" class="mc-grid"/><line x1="40" x2="${W - 8}" y1="${mid + step * k}" y2="${mid + step * k}" class="mc-grid"/>
        <text x="34" y="${mid - step * k + 4}" class="mc-ax">+${step}</text><text x="34" y="${mid + 4}" class="mc-ax">0</text><text x="34" y="${mid + step * k + 4}" class="mc-ax">−${step}</text>
        ${bars}<line x1="40" x2="${W - 8}" y1="${mid}" y2="${mid}" class="mc-base"/></svg><div class="mc-tip" hidden></div></div>
      <ul class="mc-key"><li><i class="mc-w"></i>Win</li><li><i class="mc-l"></i>Loss</li><li><i class="mc-d"></i>Draw</li></ul>
    </section>`;
  }
  // The chart's tooltip follows the pointer (or a tap on phones).
  function wireMarginChart(){
    const wrap = app.querySelector(".mc-wrap"); if(!wrap) return;
    const tip = wrap.querySelector(".mc-tip");
    const show = e => {
      const g = e.target.closest(".mc-bar"); if(!g){ tip.hidden = true; return; }
      const b = wrap.getBoundingClientRect(), r = g.getBoundingClientRect();
  ...
  async function viewWarBattles(){
    ...
    // A game's detachments are its list's (as the list is now).
    const detOf = g => { const l = g.listId && D.lists.find(x => x.id === g.listId); return l ? detText(l) : ""; };
    // Most valuable units: how often each was picked, and how those games went.
    function mvpTable(gs){
      const rows = group(gs.filter(g => g.mvp), g => g.mvp, k => { const u = D.units.find(x => x.id === k); return u ? `<button type="button" class="linkish" data-unit="${esc(u.id)}">${esc(u.name)}</button>` : "A deleted unit"; });
      return rows.length ? `<section class="panel wrec"><h2 class="ph">Most valuable units</h2><div class="wt-scroll"><table class="wtable compact">
  ...
  async function viewLiveryNew(){
    ...
    if(store.canWrite){
      ...
      $("f-import-army").addEventListener("change", async e => {
        ...
        if(!file) return;
        try {
          // The same restore as Settings uses, so photos, lists and battles come back too.
          const b = readBackup(JSON.parse(await file.text()));
          if(!b || !b.ledgers.length) throw new Error("not a ledger");
          b.ledgers = b.ledgers.map(L => ({...L, faction: FBY[L.faction] ? L.faction : "space-marines"}));
          const r = await restoreBackup(b);
          location.hash = r.firstArmy && !(b.ledgers[0].scheme || {}).pool ? "#/army/" + r.firstArmy : "#/livery/ledgers";
  ...
  function openImportArmy(pre = {}){
    ...
    d.querySelector("form").addEventListener("submit", async e => {
      ...
      try {
        ...
        try {
          const l = await store.saveList({armyId: army.id, name, limit, size: sizeOf({limit}), detachments: parsedDets(parsed), status: "draft", units: entries});
          go = `#/war/list/${l.id}`;
  ...
  const statCells = u => `<td class="n" data-label="Models">${modelsOf(u)}</td><td class="n" data-label="Points">${num(+u.points || 0)}</td>`;
  ...
  function makeListReader(factionId){
    ...
    function parseList(text){
      ...
      const out = [], unmatched = [];
      let cur = null, limit = 0, softLimit = 0, baseIndent = null, detachments = [];
      ...
      lines.forEach((raw, idx) => {
        ...
        if(dm){ detachments = dm[1].trim().split(/\s+\+\s+/).map(full => { const short = full.replace(/\s*\(.*\)\s*$/, ""); return (detByName(factionId, full) || detByName(factionId, short) || {n: short}).n; }); return; }
        ...
        if(h){
          ...
          const sizeLine = /(strike force|incursion|onslaught|combat patrol|detachment)/i.test(h[2]);
          const al = !sizeLine && allySheet(h[2]);
          if(al){
            finish();
          ...
          finish();
          if(!/^(characters?|battleline|other datasheets|dedicated transports?|allied units|epic hero)/i.test(h[2]) && !sizeLine && idx > 0) unmatched.push(h[2]);
          return;
        }
        if(!cur) return;
      ...
      return {units: out, unmatched, limit, detachments, detachment: detachments.join(" + ")};
    }
    return {parseCode, parseList};
  }
  /* ============================================================
...
Redemptor Dreadnought (210 points)</pre>
      ...
      <section class="lp-war panel" aria-labelledby="lp-war-h">
        ...
        <div class="lpw-art" aria-hidden="true">
          <div class="ill-stats">
            ...
            <div class="rdy-tile"><b>6</b><small>Lists</small></div>
          </div>
          <div class="ill-card lpw-list">
            <div class="ill-rh"><strong>Club night · 2,000 pts</strong><small>List</small></div>
            <div class="ill-prog"><i style="width:99%"></i></div>
            <p class="lpw-verdict">1,990 of 2,000 pts · 10 left</p>
            <div class="ill-row"><span>Redemptor Dreadnought</span><em>210 pts</em></div>
            <div class="ill-row"><span>4× Intercessors</span><em>320 pts</em></div>
          </div>
    ...
    const warBody = () => `
      <section class="lp-features" id="features" aria-labelledby="lp-feat-h">
        <div class="lp-head">
          <p class="eyebrow">What you get</p>
          <h2 id="lp-feat-h">Your command centre, from first idea to tabletop</h2>
          <p class="sub">Try out any army, build lists from it and see how every game went.</p>
        </div>
        ...
        <article class="lp-feat">
          ...
          ${shot("war-collection", "War Ledger's collection", `<div class="ill ill-wcoll">
            <div class="ill-stats"><div><b>4</b><small>Armies</small></div><div><b>247</b><small>Models</small></div><div class="rdy-tile"><b>4,850</b><small>Points</small></div></div>
            <div class="ill-card">
              <div class="ill-force">${[["Captain", "Character", 1, 80], ["Intercessor Squad", "Battleline", 10, 160], ["Gladiator Lancer", "Vehicle", 1, 160]].map(([n, r, m, p]) => `<div class="ill-row"><span>${n}<small>${r}</small></span><em>${m} ${m === 1 ? "model" : "models"}</em><span>${p}</span></div>`).join("")}</div>
            </div>
          </div>`)}
        </article>
        ...
        <article class="lp-feat flip">
          <div class="lp-text">
            ...
            <p>Each army gets its own command page: points against its limit, models, games played and its win–loss record, with its force broken down by role and a table of every unit.</p>
            <ul class="lp-list"><li>${tick}Force composition by role</li><li>${tick}Group and sort units your way</li><li>${tick}Select several units to move, star or add to a list</li></ul>
          </div>
          ${shot("war-army", "An army's current force in War Ledger", `<div class="ill ill-wforce">
            <div class="ill-card">
              ...
              <div class="ill-force">${[["Captain", "Character", 1, 80], ["Intercessor Squad", "Battleline", 10, 160], ["Terminator Squad", "Infantry", 5, 170], ["Redemptor Dreadnought", "Vehicle", 1, 210]].map(([n, r, m, p]) => `<div class="ill-row"><span>${n}<small>${r}</small></span><em>${m} ${m === 1 ? "model" : "models"}</em><span>${p}</span></div>`).join("")}</div>
            </div>
          </div>`)}
        </article>
        ...
        <article class="lp-feat">
          ...
          ${shot("war-list", "An army list against its points limit", `<div class="ill ill-wlist">
            <div class="ill-card lpw-list">
              ...
              <div class="ill-prog"><i style="width:99%"></i></div>
              <p class="lpw-verdict">10 pts left · Gladius Task Force</p>
              <div class="ill-row"><span>Redemptor Dreadnought</span><em>210 pts</em></div>
              <div class="ill-row"><span>4× Intercessors</span><em>320 pts</em></div>
              <div class="ill-row"><span>Gladiator Lancer</span><em>160 pts</em></div>
            </div>
          </div>`)}
        </article>
  ...
  async function openRestore(b){
    ...
    const d = modal("Restore from a file", `
      <p class="sub">${b.exported ? `This backup was made on ${esc(dayText(String(b.exported).slice(0, 10)))}. ` : ""}What you pick is added alongside what's already here. Nothing is replaced or deleted.</p>
  ...
  function profileEdits(){
    ...
    onWin("click", e => { if(menu.isConnected && !e.target.closest(".ph-avatar")) menuOpen(false); }, document);
  ...
  async function viewSetup({factionId, armyId}){
    ...
    async function saveSetup(){
      ...
      try {
        ...
        // A second save on this page compares against these colours and ranks.
        if(editing){ army.scheme = JSON.parse(JSON.stringify(sch)); tierFrom = sch.tiers.map((x, i) => i); }
    ...
    async function followColours(before, after){
      ...
      // Each old rank's place now: a deleted rank's units go to the first rank.
      const newAt = old.tiers.map((t, i) => { const j = tierFrom.indexOf(i); return j < 0 ? 0 : j; });
      const renumber = newAt.some((j, i) => j !== i);
      const tierMoved = old.tiers.map((o, i) => { const t = after.tiers[newAt[i]]; return !!t && (o.color !== t.color || (o.paint || "") !== (t.paint || "")); });
      if(!keys.length && !renumber && !tierMoved.some(Boolean)) return 0;
      let n = 0;
      for(const u of await store.listUnits(army.id)){
        ...
        const i = Math.min(u.tier || 0, old.tiers.length - 1), j = i < 0 ? 0 : newAt[i];
        if(i >= 0 && tierMoved[i]) follow("helmet", old.tiers[i].color, old.tiers[i].paint || "", after.tiers[j].color, after.tiers[j].paint || "");
        if((u.tier || 0) !== j){ u.tier = j; touched = true; }
    ...
    async function onClick(e){
      ...
      if(t.id === "s-del"){
        if(!armed){ armed = true; t.classList.add("armed"); t.textContent = "Press again to delete ledger and all its units"; return; }
        t.disabled = true;
  ...
  async function viewLedger(armyId, openUnit){
    ...
    app.innerHTML = `
      ...
      <dialog id="editdlg" class="editdlg" aria-labelledby="ed-title">
        <form id="form" class="editor-form" autocomplete="off" novalidate>
          ...
          <fieldset>
            ...
            <div class="full painted-row"><label for="f-painted">Models painted</label>
      ...
      <dialog id="listdlg" aria-labelledby="ld-h">
        <div class="dlg-close"><button type="button" data-close>Close</button></div>
        <div class="listimp">
          <h2 id="ld-h">Add units from a list</h2>
    ...
    function render(){
      ...
      if(!list.length){ c.innerHTML = `<div class="empty">${units.length ? (filter === "fav" && !query ? "No starred units yet. Tap the star on a unit to keep it here." : "No units match.") : canWrite ? "No units yet. Add one with + Add unit, or paste an army list with Add from a list." : "No units in this ledger yet."}</div>`; }
      else {
        ...
        c.innerHTML = grouped ? groups.map(([name, us]) => `<section class="grp" style="--n:${Math.min(us.length, 4)}">
            <h3 class="group-h"><span>${esc(name)}</span><small>${plural(us.length, "unit")} · ${num(us.reduce((a, u) => a + (u.points || 0), 0))} pts · ${us.reduce((a, u) => a + u.painted, 0)}/${us.reduce((a, u) => a + u.count, 0)} painted</small></h3>
      ...
      $("st-models").textContent = plural(models, "model") + (units.length > ownedHere.length ? ` · ${units.length - ownedHere.length} planned` : "");
      $("st-fin").textContent = ownedHere.length ? `${fin} of ${plural(ownedHere.length, "unit")} finished` : "Nothing to paint yet";
    ...
    form.addEventListener("submit", async e => {
      e.preventDefault();
      const wasNew = !currentUnit(), name = unitNameNow();
    ...
    $("b-del").addEventListener("click", async () => {
      ...
      const b = $("b-del");
      if(!armed){ armed = true; b.classList.add("armed"); b.textContent = "Press again to delete"; return; }
    ...
    if(canWrite){
      ...
      $("bb-del").addEventListener("click", async () => {
        ...
        if(!b.classList.contains("armed")){ b.classList.add("armed"); b.textContent = `Press again to delete ${list.length}`; return; }
    ...
    async function setLimit(v){
      try {
        ...
        $("b-limit").textContent = scheme.limit ? num(scheme.limit) : "set limit";
        render();
    ...
    async function onDetailClick(e){
      ...
      if(del){
        ...
        if(!del.classList.contains("armed")){ del.classList.add("armed"); del.textContent = "Press again to delete"; return; }
    ...
    let parsed = null;
    const parsedSum = () => { const us = parsed.units; $("ld-sum").textContent = us.length ? [parsed.detachment, plural(us.length, "unit"), num(us.reduce((a, u) => a + (u.include ? u.points : 0), 0)) + "\u00a0pts"].filter(Boolean).join(" · ") : ""; };
    function renderParsed(){
      ...
      $("ld-lim-text").textContent = `Set points limit to ${num(parsed.limit)}`;
    }
    if(canWrite){
      ...
      $("ld-out").addEventListener("input", e => {
        ...
        // A different number of models costs the datasheet's points for that size (the row updates in place, so typing isn't interrupted).
        if(t.dataset.cnt != null){
          const u = parsed.units[+t.dataset.cnt], was = sheetPts(u.sheet, u.count);
          u.count = Math.min(99, Math.max(1, parseInt(t.value, 10) || 1));
          const p = sheetPts(u.sheet, u.count); if(p != null && was != null) u.points = Math.max(0, p + u.points - was);   // extras (an enhancement) stay on top
          const cell = $("ld-out").querySelector(`[data-pts="${t.dataset.cnt}"]`); if(cell) cell.textContent = u.points;
          parsedSum();
        }
      });
    ...
    if(canWrite){
      $("b-delarmy").addEventListener("click", () => confirmDeleteArmy(army, units, () => { setDirty(false); view.guard = null; location.hash = "#/livery"; }));
    }
    ...
    view.cleanup = () => {
      ...
      window.removeEventListener("beforeunload", onBeforeUnload); $("detail").removeEventListener("click", onDetailClick); $("detail").removeEventListener("change", onDetailChange); $("detail-body").removeEventListener("toggle", onDetailToggle, true); clearPending();
    ...
    function librarySection(){
      ...
      return `<section class="lib">
        ...
        <div class="recipes">${list.map(r => `<article class="recipe lib-r">
          <header><div><h3>${esc(r.name)}</h3><small>${esc([r.area, plural(r.steps.length, "step")].filter(Boolean).join(" · "))}</small></div>
          <div class="row-actions"><button type="button" class="btn-sm primary" data-act="lib-add" data-id="${esc(r.id)}">Add</button><button type="button" class="btn-sm danger${libDelArmed === r.id ? " armed" : ""}" data-act="lib-del" data-id="${esc(r.id)}">${libDelArmed === r.id ? "Press again to delete" : "Delete"}</button></div></header>
    ...
    function updateBuyBadge(){
      ...
      [$("buy-tab")].forEach(b => { if(!b) return; b.hidden = !n; b.textContent = n; });
    }
    ...
    async function addOwned(){
      ...
      await PU.load(); const hit = PU.find(v) || PU.search(v, [], 1)[0];
      const label = hit && PU.norm(hit.label) === PU.norm(v) ? hit.label : v;
    ...
    $("paintdlg").addEventListener("click", async e => {
      ...
      if(tab){ libDelArmed = ""; suggesting = null; editingRecipe = null; pdTab = tab.dataset.tab; renderPaints(); return; }
      ...
      if(act === "new-recipe"){ editingRecipe = {id: S.newId(), name: "", area: "", notes: "", steps: [{t: "Prime", p: ""}, {t: "Basecoat", p: ""}, {t: "Shade / wash", p: ""}, {t: "Edge highlight", p: ""}], isNew: true}; renderPaints(); }
        ...
        if(!delArmed){ delArmed = true; b.classList.add("armed"); b.textContent = library ? "Press again to remove" : "Press again to delete"; return; }
  ...
  function drawRoster(){
    ...
    if(!units.length){
      $("ro-body").innerHTML = `<div class="ro-empty"><strong>No units yet</strong><p>Open a ledger and add your units, or paste an army list, and they'll all show up here.</p></div>`;
      return;
    }
